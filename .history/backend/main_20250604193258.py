...
async def submit_preferences_v2_enhanced(request: Request, db: AsyncSession = Depends(get_db)):
    """Căutare îmbunătățită cu AI recommendations INCLUSE în rezultatele căutării"""
    ...
    include_ai_recommendations = data.get("include_ai_recommendations", True)  # NEW: Flag to include AI
    
    print(f"Enhanced context-aware search v2: {city} - {activities} - {time} (ML: {enhanced_ml}, AI in results: {include_ai_recommendations})")
    ...
    # Logica pentru Amadeus API (păstrăm cum era)
                ...
                for item in api_data:
                    ...
                    if matches_category:
                        amadeus_results.append({
                            ...
                            "data_source": "amadeus",
                            "recommendation_source": "search",  # NEW: Mark as search result
    ...
    # NEW: Get AI recommendations and include in search results
    ml_recommendations = []
    if include_ai_recommendations and (user_id or session_id):
        try:
            # Get search context for context-aware recommendations
            search_context = ml_recommender.get_search_context(session_id)
            
            # Get AI recommendations (limit to fewer since we're mixing with Amadeus)
            ml_data = await ml_recommender.get_home_recommendations(
                db, user_id, session_id, 6, search_context  # Get 6 AI recommendations
            )
            
            if ml_data and ml_data.get('main_recommendations'):
                ml_recommendations = ml_data['main_recommendations']
                
                # Enhanced marking of ML recommendations
                for rec in ml_recommendations:
                    ...
                    # Ensure recommendation_source is set properly
                    if not rec.get('recommendation_source'):
                        rec['recommendation_source'] = 'ai'
                    
                    # Add search-specific reasoning if context exists
                    if search_context:
                        if rec.get('recommendation_reason'):
                            rec['recommendation_reason'] = f"Perfect for your {city} search: {rec['recommendation_reason']}"
                        else:
                            rec['recommendation_reason'] = f"AI picked this for your {city} search"
                
                print(f"Added {len(ml_recommendations)} context-aware ML recommendations to search results")
                
        except Exception as ml_error:
            print(f"Enhanced ML recommendations failed: {ml_error}")

    # NEW: Combine and interleave Amadeus + AI recommendations for better UX
    if ml_recommendations and amadeus_results:
        # Interleave AI and Amadeus results for better distribution
        all_recommendations = []
        max_len = max(len(ml_recommendations), len(amadeus_results))
        
        for i in range(max_len):
            # Add AI recommendation if available
            if i < len(ml_recommendations):
                all_recommendations.append(ml_recommendations[i])
            
            # Add Amadeus recommendation if available
            if i < len(amadeus_results):
                all_recommendations.append(amadeus_results[i])
    ...
    # Remove duplicates by name (prioritize AI over Amadeus if same name)
    seen_names = set()
    ...
    # Enhanced response with AI integration
    ai_count = len([r for r in unique_recommendations if r.get('isMLRecommendation')])
    amadeus_count = len([r for r in unique_recommendations if r.get('data_source') == 'amadeus'])
    ...
    return {
        ...
        "lon": lon,
        "session_id": session_id,
        "ml_enhanced": ai_count > 0,  # True if we have AI recommendations
        "context_stored": True,
        "ai_recommendations_included": ai_count > 0,  # NEW: Flag for frontend
        "total_found": len(unique_recommendations),
        "sources": {
            "amadeus": amadeus_count,
            "ai_personalized": ai_count,  # NEW: Count of AI recommendations
        ...
        "enhanced_features": {
            ...
            "ai_in_search_results": ai_count > 0,  # NEW
            "interleaved_recommendations": ai_count > 0 and amadeus_count > 0,  # NEW
            "diversity_enforcement": True
        },
        "search_metadata": {
            "enhanced_ml_version": "2.1.0_context_aware_with_ai_in_search",
            "algorithm": "interleaved_ai_amadeus",  # NEW
            "personalization_applied": user_id is not None,
            "session_learning": session_id is not None,
            "context_stored_for_recommendations": True,
            "ai_recommendations_in_results": ai_count > 0,  # NEW
            "recommendation_mix": f"{ai_count}_ai_{amadeus_count}_amadeus"  # NEW
        }
    }

# NEW: Endpoint to get AI recommendations separately (optional)
@app.get("/ml/search-recommendations")
async def get_search_ai_recommendations(
    user_id: Optional[str] = None,
    session_id: Optional[str] = None,
    city: Optional[str] = None,
    limit: int = 6,
    ...
    """Get AI recommendations specifically for search results mixing"""
    
    try:
        print(f"Getting AI recommendations for search mixing: {user_id or 'anonymous'}")
        
        # Get search context if available
        search_context = None
        if session_id:
            search_context = ml_recommender.get_search_context(session_id)
            if search_context and city:
                # Update context with current city if provided
        ...
        # Mark all as AI recommendations for search mixing
        if recommendations.get('main_recommendations'):
            for rec in recommendations['main_recommendations']:
                rec['isMLRecommendation'] = True
                rec['enhanced_ml'] = True
                rec['context_aware'] = search_context is not None
                rec['ml_enhanced'] = True
                rec['for_search_mixing'] = True
        
        return {
            "status": "success",
            "recommendations": recommendations.get('main_recommendations', []),
            "metadata": {
                "context_influenced": search_context is not None,
                "city_filter": city,
                "purpose": "search_mixing",
                "ml_version": "2.1_search_ai"
            }
        }
        
    except Exception as e:
        print(f"Error getting search AI recommendations: {e}")
        return {
            "status": "error",
            "recommendations": [],
...
async def startup_event():
    ...
    print("NEW: AI recommendations in search results activated!")
...
async def health_check(db: AsyncSession = Depends(get_db)):
    try:
        ...
        return {
            "status": "healthy",
            "version": "2.1.0_context_aware_with_ai_in_search",
            "database": "connected",
            ...
            "ml_features": {
                ...
                "ai_in_search_results": True     # NEW
            },
            "api_features": {
                "home_recommendations": "context_aware_personalized",  
                "search_enhancement": "ai_integrated_interleaved",     # UPDATED
                "real_time_updates": "enabled",
                "user_insights": "available",
                "trending_analysis": "active",
                "context_management": "active",                        
                "ai_search_mixing": "active"                           # NEW
            },
            "performance": {
                "recommendation_latency": "< 200ms",
                "learning_speed": "immediate",
                "personalization_accuracy": "high",
                "context_influence": "real_time",                      
                "ai_search_integration": "seamless"                    # NEW
            },
            "timestamp": datetime.utcnow().isoformat()
        }
    except Exception as e:
