...
@router.get("/place-details/{xid}")
async def get_place_details_by_xid(xid: str):
    url = f"{DETAIL_URL}{xid}"
    async with httpx.AsyncClient() as client:
        response = await client.get(url, params={"apikey": API_KEY})

    if response.status_code != 200:
        return JSONResponse(status_code=response.status_code, content={"error": "Failed to load details"})
