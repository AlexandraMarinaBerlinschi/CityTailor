...
async def initialize_ml_user(
    ...
    try:
        ...
        if not user_exists:
            print(f"⚠️  User {user_id} not found in auth.users")
            return {
                "status": "warning",
    ...
    try:
        ...
        if migrated_count > 0:
            try:
                ...
                print(f"⚠️  Failed to update profile after migration: {profile_error}")
...
async def delete_user_ml_data(
    ...
    try:
        ...
        print(f"🗑️  Completely deleted ML data for user {user_id[:8]}...")
