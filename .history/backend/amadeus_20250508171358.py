...
@router.get("/places")
async def get_tours(city: str = Query(...), radius: int = Query(10)):
    try:
        radius = int(radius)
    except ValueError:
        return JSONResponse(status_code=400, content={"error": "Radius must be an integer"})
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            print("Tours API error:", response.text)
