...
# Fixes pentru main.py - Endpoint submit-preferences-v2
...
async def submit_preferences_v2_enhanced(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    session_id = data.get("session_id") or str(uuid.uuid4())
    user_id = data.get("user_id")
    enhanced_ml = data.get("enhanced_ml", True)  # Changed: Default to True
    include_ai_recommendations = data.get("include_ai_recommendations", True)
    
    print(f"Enhanced search v2: {city} - {activities} - ML: {enhanced_ml}, AI in results: {include_ai_recommendations}")
    
    # IMPORTANT: Track search FIRST pentru a stoca contextul
    try:
        await ml_recommender.track_search(
            db=db,
            user_id=user_id,
        ...
        print("ML tracking successful - context stored")
    except Exception as tracking_error:
        print(f"ML tracking failed: {tracking_error}")
    
    # NEW: Obține AI recommendations IMEDIAT după tracking (cu contextul fresh)
    ml_recommendations = []
    if include_ai_recommendations and (user_id or session_id):
        try:
            ...
            ml_data = await ml_recommender.get_home_recommendations(
                db, user_id, session_id, 8, search_context  # Increase to 8 AI recommendations
            )
            
            if ml_data and ml_data.get('main_recommendations'):
                ml_recommendations = ml_data['main_recommendations']
                
                # Mark as AI recommendations pentru frontend
                for rec in ml_recommendations:
                    ...
                    # Add search-specific reasoning
                    if search_context:
                        if not rec.get('recommendation_reason'):
                            rec['recommendation_reason'] = f"AI selected this for your {city} search"
                        else:
                            rec['recommendation_reason'] = f"Perfect for {city}: {rec['recommendation_reason']}"
                
                print(f"✅ Added {len(ml_recommendations)} AI recommendations to search results")
            else:
    ...
    # Continue cu Amadeus API call...
    geo = await get_lat_lon_by_city(city)
    if not geo:
        # Return AI recommendations even if Amadeus fails
        if ml_recommendations:
            return {
                ...
                "total_found": len(ml_recommendations),
                "sources": {
                    "amadeus": 0,
                    "ai_personalized": len(ml_recommendations)
                },
                "message": "AI recommendations available (location service unavailable)"
            }
    ...
    # Only get Amadeus if we have token
    if token:
        try:
            ...
            params = {
                ...
                "longitude": lon,
                "radius": 15,
                "page[limit]": 30  # Reduced to make room for AI
            }
    ...
    # NEW: Prioritize AI recommendations at the beginning
    ...
    return {
        ...
        "enhanced_features": {
            ...
            "ai_first_priority": True,  # NEW
            "immediate_ai_integration": True  # NEW
        },
        "search_metadata": {
            "enhanced_ml_version": "2.1.0_ai_first",
            "algorithm": "ai_priority_search",
