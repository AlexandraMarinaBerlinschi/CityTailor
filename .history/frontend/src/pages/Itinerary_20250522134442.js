      ...
      if (location.state?.itineraryItems && location.state.itineraryItems.length > 0) {
        ...
        const initialItinerary = location.state.itineraryItems.map(item => {
          ...
          return {
            ...
            city: location.state.city || item.city || ""
          };
        }).filter(item => 
        ...
        console.log("✅ Processed navigation itinerary:", initialItinerary);
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-4 p-2 bg-gray-100 rounded text-sm">
        ...
        <p>Debug: Has valid map data: {hasValidMapData ? 'Yes' : 'No'}</p>
      </div>
      ...
      <div className="mb-6">
        {hasValidMapData && itinerary.length > 0 ? (
          <MapContainer 
            ...
            zoom={mapZoom} 
            style={{ height: "400px" }}
            key={`map-${mapCenter[0]}-${mapCenter[1]}-${itinerary.length}`}
          >
            <TileLayer
            ...
            {itinerary.map((item, index) => {
              ...
              return (
                <Marker key={`${item.id}-${index}`} position={[item.lat, item.lon]}>
                  <Popup>
                    <div>
