...
const Itinerary = () => {
  ...
  const getStorageKey = (currentUserId) => {
    return currentUserId ? `pendingItinerary-${currentUserId}` : "pendingItinerary-guest";
  };

  // Helper function to clear all itinerary storage
  const clearAllItineraryStorage = () => {
    // Clear guest storage
    sessionStorage.removeItem("pendingItinerary-guest");
    
    // Clear any user-specific storage (we might not know all user IDs)
    Object.keys(sessionStorage).forEach(key => {
      if (key.startsWith("pendingItinerary-")) {
  ...
  const checkAuthentication = async () => {
    ...
    if (!token) {
      ...
      // Clear user data and switch to guest mode
      if (isAuthenticated || userId) {
        console.log("🔄 Switching from authenticated to guest mode");
        setIsAuthenticated(false);
        setUserId(null);
        resetItineraryState();
        clearAllItineraryStorage();
      }
      return false;
    }
    ...
    try {
      ...
      if (userResponse.ok) {
        ...
        // Clear invalid authentication and switch to guest mode
        if (isAuthenticated || userId) {
      ...
      // Clear authentication on error and switch to guest mode
      if (isAuthenticated || userId) {
  ...
  const loadItineraryData = async (authenticatedUserId = null) => {
    ...
    // First check for data from navigation state (from Recommendations page)
    if (location.state?.itineraryItems && location.state.itineraryItems.length > 0) {
      ...
      // Get existing items from sessionStorage using the correct key
      let existingItems = [];
      const currentStorageKey = getStorageKey(authenticatedUserId);
      const existingItinerary = sessionStorage.getItem(currentStorageKey);
      ...
      // Update sessionStorage with merged data using correct key
      sessionStorage.setItem(currentStorageKey, JSON.stringify({
        items: validMergedItems,
        title: title
      }));
      
      return; // Exit early since we found navigation data
    }

    // If authenticated, try to load saved itinerary from server
        ...
        if (itineraryResponse.ok) {
          ...
          if (itineraryData && itineraryData.length > 0) {
            ...
            if (activitiesResponse.ok) {
              ...
              // Also save to sessionStorage for the current user
              const currentStorageKey = getStorageKey(authenticatedUserId);
              sessionStorage.setItem(currentStorageKey, JSON.stringify({
    ...
    // Fallback: Check sessionStorage with the correct key
    const currentStorageKey = getStorageKey(authenticatedUserId);
    const pendingItinerary = sessionStorage.getItem(currentStorageKey);
    ...
    if (pendingItinerary) {
      ...
      // No data found - clear the itinerary
      console.log("🔄 No itinerary data found, starting fresh");
      resetItineraryState();
    }
  };
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      ...
      // Load itinerary data (whether authenticated or not)
  ...
  const handleOnDragEnd = (result) => {
    ...
    // Update sessionStorage with reordered items using correct key
  ...
  const removeFromItinerary = (index) => {
    ...
    // Update sessionStorage using correct key
    const storageKey = getStorageKey(userId);
    sessionStorage.setItem(storageKey, JSON.stringify({
      items: updated,
      title: title
  ...
  const handleTitleChange = (newTitle) => {
    ...
    // Update sessionStorage with new title
    const storageKey = getStorageKey(userId);
    sessionStorage.setItem(storageKey, JSON.stringify({
      items: itinerary,
      title: newTitle
    ...
    <div className="p-4">
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-center gap-2 mb-2">
          ...
          {isAuthenticated ? (
            <span className="text-green-600 font-medium">✅ Auto-saving enabled (User ID: {userId})</span>
          ) : (
            <span className="text-gray-600">📝 Guest mode - Changes saved locally only</span>
          )}
        </div>
