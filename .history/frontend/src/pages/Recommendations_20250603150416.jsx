...
const isUserAuthenticated = () => {
  ...
  // This could be checking for auth token, user session, etc.
  return localStorage.getItem('authToken') || localStorage.getItem('user') || sessionStorage.getItem('isLoggedIn');
};
...
const Recommendations = () => {
  ...
  // Helper function to get recommendations from storage with user context
  const getStoredRecommendations = () => {
    if (!isUserAuthenticated()) {
      return [];
    }
    const userSpecificKey = getUserStorageKey("latestRecommendations");
    return JSON.parse(sessionStorage.getItem(userSpecificKey)) || [];
  };

  // Helper function to get city from storage with user context
  const getStoredCity = () => {
    if (!isUserAuthenticated()) {
      return "";
    }
    const userSpecificKey = getUserStorageKey("latestCity");
  ...
  // Save recommendations with user-specific keys and authentication check
  useEffect(() => {
    if (raw.length > 0 && isUserAuthenticated()) {
      const userSpecificRecommendationsKey = getUserStorageKey("latestRecommendations");
      const userSpecificCityKey = getUserStorageKey("latestCity");
      
      sessionStorage.setItem(userSpecificRecommendationsKey, JSON.stringify(raw));
      sessionStorage.setItem(userSpecificCityKey, city);
    }
  }, [raw, city]);

  // Clear user-specific data on logout/component unmount if not authenticated
  useEffect(() => {
    return () => {
      if (!isUserAuthenticated()) {
        // Clear all user-specific session storage when not authenticated
        const keysToRemove = [];
        for (let i = 0; i < sessionStorage.length; i++) {
          const key = sessionStorage.key(i);
          if (key && (key.includes('latestRecommendations_') || key.includes('latestCity_'))) {
  ...
  // Show empty state if user is not authenticated and no current recommendations
  if (!isUserAuthenticated() && raw.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🔐</div>
          <h3 className="text-2xl font-bold text-gray-900 mb-2">Please log in to view recommendations</h3>
          <p className="text-gray-600 mb-6">
            Your personalized recommendations and favorites are saved securely to your account.
          </p>
          <button
            onClick={() => navigate('/login')}
            className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 mr-4"
          >
            Log In
          </button>
          <button
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      {/* Empty State */}
      {raw.length === 0 && isUserAuthenticated() && (
        <div className="text-center py-12">
          <div className="text-6xl mb-4">🔍</div>
