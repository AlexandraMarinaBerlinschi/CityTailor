...
import { useNavigate } from "react-router-dom"; // asigură-te că ai acest import
        ...
        <div className="max-w-2xl mx-auto">
            ...
            <div className="space-y-4">
              <div className="flex items-center p-4 bg-blue-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-800">{submittedData.city}</p>
                </div>
              </div>

              <div className="flex items-start p-4 bg-purple-50 rounded-lg">
                <Activity className="w-5 h-5 text-purple-600 mr-3 mt-1" />
                <div className="flex-1">
                  <span className="text-sm text-gray-600">Preferred Activities:</span>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {submittedData.activities.map((activity, index) => (
                      <span
                        key={index}
              ...
              <div className="flex items-center p-4 bg-green-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-800">
                    {timeOptions.find(opt => opt.id === submittedData.time)?.label}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-r from-blue-500 to-purple-500 rounded-2xl p-8 text-white text-center mb-8">
            ...
            <p className="text-blue-100 mb-6">
              Based on your preferences for <strong>{submittedData.city}</strong>, we're creating a personalized itinerary
              that matches your interests and available time.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
              <div className="bg-white bg-opacity-20 rounded-lg p-4">
                <div className="text-2xl mb-2">🔍</div>
                <div className="font-semibold">Analyzing</div>
                <div className="text-sm text-blue-100">Local attractions</div>
              </div>
              <div className="bg-white bg-opacity-20 rounded-lg p-4">
                <div className="text-2xl mb-2">📅</div>
                <div className="font-semibold">Planning</div>
                <div className="text-sm text-blue-100">Your schedule</div>
              </div>
              <div className="bg-white bg-opacity-20 rounded-lg p-4">
                <div className="text-2xl mb-2">✨</div>
                <div className="font-semibold">Customizing</div>
          ...
          <div className="text-center space-y-4">
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <button
                onClick={() => {
                  // Redirect către pagina de recomandări cu parametrii
                  const params = new URLSearchParams({
                    city: submittedData.city,
                    activities: submittedData.activities.join(','),
                    time: submittedData.time
                  });
                  window.location.href = `/recommendations?${params.toString()}`;
                }}
                className="inline-flex items-center px-10 py-4 text-xl font-bold text-white bg-gradient-to-r from-emerald-500 to-blue-600 rounded-full shadow-xl hover:shadow-2xl transform transition-all duration-300 hover:scale-110 hover:from-emerald-600 hover:to-blue-700 group relative overflow-hidden"
              >
              ...
              <button
                onClick={handleReset}
                className="inline-flex items-center px-6 py-3 text-lg font-semibold text-gray-700 bg-white border-2 border-gray-300 rounded-full shadow-lg hover:shadow-xl transform transition-all duration-300 hover:scale-105"
              >
            ...
            <div className="mt-8 p-6 bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl border border-amber-200">
              <div className="flex items-center justify-center mb-3">
                ...
                <h4 className="text-lg font-semibold text-amber-800">Ready for the next step?</h4>
              </div>
              <p className="text-amber-700 text-center leading-relaxed">
                Click <strong>"Explore Recommendations"</strong> to see personalized attractions, restaurants, and activities for <strong>{submittedData.city}</strong>. 
                <br />You'll then be able to build your perfect itinerary!
              </p>
            </div>
          </div>
        </div>
