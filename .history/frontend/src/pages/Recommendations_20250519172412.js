  ...
  const goToItineraryPage = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/itineraries/", {
        ...
        body: JSON.stringify({
          city: city,
          name: "Itinerariul meu",
          activities: [],
        }),
      });

      if (!res.ok) throw new Error("Eroare la creare itinerariu");
      ...
      console.error(err);
      alert("Eroare la salvarea itinerariului.");
    }
  };
        ...
        <div className="mb-6 p-4 bg-blue-50 rounded-lg shadow">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-medium text-blue-800">
              Itinerariul tău ({itinerary.length} locații)
            </h3>
            <button
          ...
          <ul className="list-disc pl-5">
            ...
            {itinerary.length > 3 && <li className="text-blue-700">...și {itinerary.length - 3} locații în plus</li>}
          </ul>
        </div>
      )}
              ...
              .map((item, index) => (
                ...
                >
                  <Popup>
                    ...
                    {item.minimumDuration && <><br />Durata: {item.minimumDuration}</>}
