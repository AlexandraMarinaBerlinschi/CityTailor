...
const Recommendations = () => {
  ...
  // Fallback dacă location.state nu există
  ...
  // Salvează fallback-ul în sessionStorage
  useEffect(() => {
    if (raw.length > 0) {
  ...
  const addToItinerary = (place) => {
    if (place && !itinerary.some((p) => p.name === place.name)) {
      const placeWithId = {
        ...place,
        id: place.id || `place-${Date.now()}-${Math.random()}` // Ensure each place has an ID
      };
      ...
      // Save to sessionStorage with better structure
