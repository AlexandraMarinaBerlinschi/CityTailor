...
const Itinerary = () => {
  ...
  const loadItineraryData = async (authenticatedUserId = null) => {
    ...
    if (location.state?.itineraryItems && location.state.itineraryItems.length > 0) {
      ...
      let existingItems = [];
      const currentStorageKey = authenticatedUserId ? `pendingItinerary-${authenticatedUserId}` : "pendingItinerary-guest"; 
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      ...
      // Clean up guest storage if user is authenticated
      if (authenticatedUserId) {
  ...
  useEffect(() => {
    if (isAuthenticated && itinerary.length > 0) {
  ...
  }, [itinerary, title, isAuthenticated]);
