...
const MapUpdater = ({ center }) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.warn("❌ MapUpdater: coordonate invalide sau lipsă:", center);
    }
  }, [center, map]);
...
const Recommendations = () => {
  ...
  const recommendations = recommendationsRaw.filter((item) => {
    ...
    if (!valid) console.warn("⚠️ Coordonate invalide în listă:", item);
    return valid;
  });

  const handleSelectPlace = (place) => {
    console.log("🧭 handleSelectPlace primit:", place);
    const lat = safeToNumber(place.lat);
    const lon = safeToNumber(place.lon);
    console.log("✅ lat/lon convertit:", lat, lon);
  ...
  return (
    <div className="max-w-5xl mx-auto p-8 relative">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        {selectedPlace &&
          safeToNumber(selectedPlace.lat) !== null &&
          safeToNumber(selectedPlace.lon) !== null && (
