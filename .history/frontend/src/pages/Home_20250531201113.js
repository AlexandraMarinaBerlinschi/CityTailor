...
const Home = () => {
  ...
  const fetchMLRecommendations = async () => {
    try {
      ...
      const sessionId = trackingService.sessionId; // Use tracking service session
      ...
      if (response.ok) {
        ...
        // Track successful ML recommendations load
  ...
  // Track place view when user clicks/views a place
  ...
  // Track add to itinerary
  const handleAddToItinerary = async (place) => {
    try {
      // Add to localStorage itinerary
  ...
  const RecommendationCard = ({ place, index }) => (
    ...
    >
      ...
      {/* Detalii cu iconuri mai expresive */}
      ...
      {/* Categorie și Scor cu design îmbunătățit */}
      ...
      {/* Action Buttons cu efecte hover îmbunătățite */}
