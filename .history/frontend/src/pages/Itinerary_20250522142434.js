...
const Itinerary = () => {
  ...
  const saveItinerary = async () => {
    // This function now only runs for authenticated users
    // since the button is only shown for authenticated users
    const token = localStorage.getItem("token");
    
    if (!token) {
      console.error("Save attempted without token - this should not happen");
    ...
    try {
      ...
      const originalButtonText = "Saving...";
      ...
      alert("⚠️ An error occurred while saving your itinerary. Please try again.");
    }
  };
