...
const Recommendations = () => {
  ...
  const city = location.state?.city || sessionStorage.getItem("latestCity") || "";
  ...
  // Effect pentru încărcarea recomandărilor
  useEffect(() => {
    const loadRecommendations = async () => {
      ...
      try {
        // Încearcă să folosească datele din location state
        if (location.state?.recommendations && location.state.recommendations.length > 0) {
          console.log("Using recommendations from navigation state");
          setRecommendations(location.state.recommendations);
          setLoading(false);
          return;
        }

        // Încearcă să găsească datele din sessionStorage
        const questionnaireData = sessionStorage.getItem("questionnaireData");
        const cachedRecommendations = sessionStorage.getItem("latestRecommendations");
        ...
        // Dacă avem datele din chestionar, încearcă să faci o nouă cerere către API
        if (questionnaireData) {
          console.log("Making new API request with questionnaire data");
          const formData = JSON.parse(questionnaireData);
          ...
          try {
            ...
            // Salvează în cache
            sessionStorage.setItem("latestRecommendations", JSON.stringify(response.data));
        ...
        setError("Unable to load recommendations. Please try submitting the questionnaire again.");
        setRecommendations([]);
      } finally {
        setLoading(false);
      }
  ...
  const averagePrice = recommendations.length > 0 
    ? Math.round(recommendations.reduce((sum, item) => sum + generateConsistentPrice(item.name), 0) / recommendations.length)
    : 0;
  ...
  const handleShare = async () => {
    const shareText = `Discover ${recommendations.length} amazing experiences in ${city}!`;
    
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Recommendations for ${city}`,
          text: shareText,
  ...
  const handleViewFavorites = () => {
    ...
    const favoritePlaces = recommendations.filter(place => favorites.has(place.name));
    console.log('Your favorite places:', favoritePlaces);
  };

  const displayedPlaces = showAllPlaces ? recommendations : recommendations.slice(0, 4);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Loading recommendations...</h2>
          <p className="text-gray-600">Finding the best places for you in {city}</p>
        </div>
      </div>
    );
  }
...
<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-xl text-gray-600">
            We found <span className="font-semibold text-blue-600">{recommendations.length}</span> amazing experiences for you
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-sm text-purple-600">Send to friends</p>
              </div>
            </button>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              <div className="p-6 border-b border-gray-200">
                <h3 className="text-xl font-semibold text-gray-900">Recommended places</h3>
              </div>
                      ...
                      <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-gray-600 mb-4 line-clamp-2">
                            {item.description || "Discover this unique experience and create unforgettable memories in this wonderful destination."}
                          </p>
              ...
              {/* Show More Button */}
              {recommendations.length > 4 && !showAllPlaces && (
              ...
              <div className="bg-gradient-to-br from-pink-50 to-red-50 border-2 border-pink-200 rounded-2xl p-6 sticky top-24">
                ...
                <div className="space-y-3 mb-6">
                  {[...favorites].slice(0, 3).map((placeName, index) => {
                    const place = recommendations.find(p => p.name === placeName);
                    return (
                      <div key={index} className="flex items-center space-x-3">
                        ...
                        <button
                          onClick={() => toggleFavorite(placeName)}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      const favoritePlaces = recommendations.filter(place => favorites.has(place.name));
                      favoritePlaces.forEach(place => addToItinerary(place));
                    }}
      ...
      {recommendations.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">Try searching in another city or change your filters.</p>
          <button
            onClick={() => navigate("/")}
            className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300"
          >
