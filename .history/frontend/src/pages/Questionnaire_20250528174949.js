...
const Questionnaire = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Încearcă să trimită către API
          const response = await axios.post("http://localhost:8000/submit-preferences", formData, {        headers: {
      ...
      // Navighează către pagina de recomandări cu datele primite
      navigate("/recommendations", {
        state: {
          recommendations: response.data,
          city: formData.city,
          lat: response.data.lat || 48.8566, // Default Paris coordinates
          lon: response.data.lon || 2.3522
        }
      });
      ...
      // Dacă API-ul nu funcționează, folosește date mock și navighează oricum
      console.log("API not available, using fallback navigation");
      
      // Salvează datele în sessionStorage pentru fallback
      sessionStorage.setItem("questionnaireData", JSON.stringify(formData));
      
      // Navighează către recommendations chiar și fără API
      navigate("/recommendations", {
        state: {
          recommendations: [], // Va fi populat din sessionStorage sau API în Recommendations.jsx
          city: formData.city,
          lat: 48.8566, // Default coordinates
          lon: 2.3522,
          formData: formData // Trimite și datele formularului
        }
      });
      
      // Nu afișa eroarea utilizatorului, doar navighează
