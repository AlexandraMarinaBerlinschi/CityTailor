...
const Recommendations = () => {
  ...
  const handleViewFavorites = () => {
    ...
    // Show a detailed view of favorites
    const favoritesText = favoritePlaces.map((place, index) => 
      `${index + 1}. ${place.name}${place.rating ? ` (${Number(place.rating).toFixed(1)}⭐)` : ''}`
    ).join('\n');
    
    alert(`Your Favorite Places in ${city}:\n\n${favoritesText}`);
  };
