...
const EnhancedHome = () => {
  ...
  const fetchMLRecommendations = useCallback(async (showRefreshing = false) => {
    try {
      ...
      const data = await enhancedTrackingService.getHomeRecommendations(12);
  ...
  const loadUserInsights = useCallback(async () => {
    try {
      const insights = await enhancedTrackingService.getPersonalizedInsights();
  ...
  const handlePlaceView = async (place, index) => {
    try {
      await enhancedTrackingService.trackPlaceView(
        place.name,
      ...
      await enhancedTrackingService.trackRecommendationInteraction(
        place,
        'click',
        { position: index, source: 'home_recommendations' }
      );
    } catch (error) {
  ...
  const handleFavorite = async (place) => {
    try {
      ...
      await enhancedTrackingService.trackFavorite(
        place.name,
        place.id || place.place_id,
        place.city,
        place.lat,
      ...
      await enhancedTrackingService.trackRecommendationInteraction(
        place,
        'favorite',
        { source: 'home_recommendations', user_action: 'positive_feedback' }
      );

      console.log(`❤️ Enhanced favorite added: ${place.name}`);
      
      // Nu mai facem manual refresh - se face automat prin real-time learning
  ...
  const handleAddToItinerary = async (place) => {
    try {
      ...
      await enhancedTrackingService.trackAddToItinerary(
        place.name,
        place.id || place.place_id,
        place.city,
        place.lat,
      ...
      await enhancedTrackingService.trackRecommendationInteraction(
        place,
        'itinerary',
        { source: 'home_recommendations', user_action: 'planning_intent' }
      );

      console.log(`✈️ Enhanced itinerary add: ${place.name}`);
      
      // Real-time refresh se face automat
  ...
  // Initialize on mount
  useEffect(() => {
    enhancedTrackingService.trackPageView('Home');

    // Set user in enhanced tracking service
    const userId = UserStorage.getCurrentUserId();
    if (userId && userId !== 'anonymous') {
      enhancedTrackingService.setCurrentUser(userId);
    }
    ...
    return () => {
      enhancedTrackingService.trackPageExit('Home');
    };
  }, [fetchMLRecommendations]);

  // Listen for real-time recommendation updates
  useEffect(() => {
    const handleRecommendationUpdate = (event) => {
      console.log('🔄 Real-time recommendation update triggered:', event.detail);
      fetchMLRecommendations(true);
    };

    // Listen for ML recommendation updates
    window.addEventListener('ml-recommendations-updated', handleRecommendationUpdate);
    
    return () => {
      window.removeEventListener('ml-recommendations-updated', handleRecommendationUpdate);
    };
  }, [fetchMLRecommendations]);
  ...
  const EnhancedRecommendationCard = ({ place, index, section = "main" }) => (
    <div 
      key={`${section}-${index}`}
        ...
        <div className="max-w-7xl mx-auto p-8 relative z-10">
          <div className="text-center text-xl">
            ...
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl inline-block shadow-xl">
              <p className="font-medium">🤖 Enhanced ML system is learning your preferences...</p>
          ...
          <div className="text-center">
            <div className="bg-white/10 backdrop-blur-lg p-12 rounded-3xl shadow-2xl border border-white/20 max-w-2xl mx-auto">
              ...
              <p className="text-xl mb-8 opacity-90 leading-relaxed">
                Our enhanced AI system is ready to learn your preferences! <br/>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-sm">
                <div className="bg-white/10 p-4 rounded-xl">
                  ...
                  <p className="opacity-80">AI adapts instantly to your choices</p>
                </div>
                <div className="bg-white/10 p-4 rounded-xl">
                  <div className="text-2xl mb-2">🎯</div>
                  <h4 className="font-semibold mb-1">Smart Personalization</h4>
                  <p className="opacity-80">Recommendations improve with every click</p>
                </div>
                <div className="bg-white/10 p-4 rounded-xl">
                  <div className="text-2xl mb-2">🌟</div>
                  <h4 className="font-semibold mb-1">Serendipity Discovery</h4>
                  <p className="opacity-80">Find unexpected perfect matches</p>
                </div>
                <div className="bg-white/10 p-4 rounded-xl">
                  <div className="text-2xl mb-2">📊</div>
                  <h4 className="font-semibold mb-1">Engagement Analytics</h4>
                  <p className="opacity-80">Track your travel personality growth</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="text-sm opacity-75">
                  <p>Enhanced ML • Real-time adaptation • Collaborative filtering • Smart discovery</p>
                </div>
              </div>
  ...
  // Render enhanced ML recommendations
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 text-white relative overflow-hidden">
      {/* Floating background elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          {mlRecommendations && (
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-xl inline-block max-w-4xl">
              <p className="text-lg opacity-90 mb-2">
                {mlRecommendations.recommendation_type === 'advanced_personalized' && "🎯 Advanced AI personalization based on your unique travel profile"}
                {mlRecommendations.recommendation_type === 'advanced_session_based' && "🔍 Smart recommendations from your current exploration session"}
              ...
              {debugInfo && (
                <div className="flex flex-wrap justify-center gap-4 text-sm mt-4">
                  <div className="bg-white/10 px-3 py-1 rounded-full">
                    ...
                    <span className="font-semibold ml-1">{debugInfo.personalization_level}</span>
                  </div>
                  ...
                  <div className="bg-white/10 px-3 py-1 rounded-full">
                    <span className="opacity-75">ML Version:</span>
                    <span className="font-semibold ml-1">{debugInfo.ml_version}</span>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-300">
                    {userInsights.engagement_analysis?.level?.toUpperCase()}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-green-300">
                    {Object.keys(userInsights.user_profile?.city_preferences || {}).length}
                  </div>
        ...
        {mlRecommendations?.main_recommendations?.length > 0 && (
          <div className="mb-12">
            <div className="text-center mb-8">
              ...
              <p className="text-lg opacity-80">AI-curated destinations based on your unique preferences</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {mlRecommendations.main_recommendations.map((place, index) => (
                <EnhancedRecommendationCard key={`main-${index}`} place={place} index={index} section="main" />
              ))}
            </div>
          </div>
        )}
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {mlRecommendations.in_your_favorite_cities.map((place, index) => (
                <EnhancedRecommendationCard key={`cities-${index}`} place={place} index={index} section="cities" />
              ))}
            </div>
          </div>
        )}
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {mlRecommendations.popular_destinations.map((place, index) => (
                <EnhancedRecommendationCard key={`popular-${index}`} place={place} index={index} section="popular" />
              ))}
            </div>
          </div>
        )}
