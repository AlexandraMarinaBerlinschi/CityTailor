    ...
    >
      <Popup maxWidth={250}>
        <div className="text-sm">
          ...
          {item.minimumDuration && <div>⏱️ {item.minimumDuration}</div>}
          {item.rating && <div>⭐ {Number(item.rating).toFixed(1)}</div>}
          <button
            onClick={() => addToItinerary(item)}
            className="mt-2 py-1 px-2 rounded bg-blue-600 text-white hover:bg-blue-700 w-full"
          >
