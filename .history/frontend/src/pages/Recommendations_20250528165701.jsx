...
const Recommendations = () => {
  ...
  useEffect(() => {
    if (cityParam && activitiesParam && timeParam) {
      const activityList = activitiesParam.split(",");
      const mock = activityList.map((activity, i) => ({
        name: `${activity} Place ${i + 1}`,
        category: activity,
        description: `Top-rated ${activity.toLowerCase()} activity in ${cityParam}`,
        minimumDuration: timeParam,
        pictures: [{ url: `https://source.unsplash.com/400x300/?${activity}` }],
        lat: 48.8566 + i * 0.002,
        lon: 2.3522 + i * 0.002,
        rating: (3.5 + Math.random() * 1.5).toFixed(1),
      }));

      sessionStorage.setItem("latestRecommendations", JSON.stringify(mock));
      sessionStorage.setItem("latestCity", cityParam);
  ...
  const handleShare = async () => {
    const shareText = `Discover ${raw.length} amazing experiences in ${city}!`;
          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Discover unique experiences</p>
              </div>
            </div>
          </div>
        </div>
