...
const Recommendations = () => {
  ...
  // DEBUG: Afișează toate datele primite de la API
  console.log("📦 Date brute primite de la API:", recommendationsRaw);

  recommendationsRaw.forEach((item, index) => {
    const hasInvalidLat = typeof item.lat !== "number" || !isFinite(item.lat);
    const hasInvalidLon = typeof item.lon !== "number" || !isFinite(item.lon);

    if (hasInvalidLat || hasInvalidLon) {
      console.warn(`❌ [RECOMANDARE INVALIDĂ - index ${index}]`, {
        name: item.name,
        lat: item.lat,
        lon: item.lon,
  ...
  // Filtrare recomandări valide
  const recommendations = recommendationsRaw.filter(
    (item) =>
