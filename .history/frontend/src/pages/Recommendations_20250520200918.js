...
const Recommendations = () => {
  ...
  // Default to Paris coordinates if no valid coordinates
  ...
  // Safely handle place selection with improved validation
  ...
  // Filter recommendations to only include items with valid coordinates
  const filteredRecommendations = recommendations.filter(item => {
    const itemLat = toNumber(item.lat);
    const itemLon = toNumber(item.lon);
    return isValidCoordinate(itemLat) && isValidCoordinate(itemLon);
  });
