...
const PlaceCard = ({ name, lat, lon, rating, pictures, minimumDuration, onClick }) => {
  const fallbackImage =
  ...
  const validLatLon = isFinite(parsedLat) && isFinite(parsedLon);
  const formattedRating = rating ? Number(rating).toFixed(1) : null;

  return (
    <div 
      className="bg-white rounded shadow p-4 mb-6 flex flex-col md:flex-row gap-4 hover:shadow-lg transition-shadow duration-300 cursor-pointer"
      onClick={onClick}
    >
      <img
        src={imageUrl}
      ...
      <div className="flex-1">
        ...
        <div className="mt-4 flex justify-between items-center">
          <button 
            className="flex items-center gap-1 px-3 py-1 border border-blue-600 bg-blue-50 text-blue-600 rounded hover:bg-blue-100 transition"
            onClick={(e) => {
              e.stopPropagation();
              onClick();
            }}
          ...
          {validLatLon && (
            <a 
              href={`https://www.google.com/maps/search/?api=1&query=${parsedLat},${parsedLon}`}
              target="_blank" 
