...
const Navbar = ({ isAuthenticated, setIsAuthenticated, user, setUser }) => {
  ...
  const handleLogout = () => {
    ...
    // 1. Clear ML tracking data FIRST (before clearing user data)
    trackingService.clearTrackingData();
    
    // 2. Clear session data (recommendations, searches)
    UserStorage.clearSessionDataOnLogout();
    
    // 3. Clear user storage data (but keep favorites/itinerary for next login)
